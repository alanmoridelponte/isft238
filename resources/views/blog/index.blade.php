                ...
                <form method="GET" action="{{ route('blog.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="relative col-span-1 md:col-span-3">
                        <input type="text" id="busqueda" name="busqueda" placeholder="Ingrese palabras claves..."
                            ...
                            class="w-full pl-10 pr-4 py-2 rounded-lg border border-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-500" />
                        <i class="fas fa-search absolute left-3 top-3 text-amber-700 pointer-events-none"></i>
                    </div>

                    <!-- Botón -->
                    <div class="flex items-end">
                        <button type="submit"
                            class="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 rounded-lg shadow transition">
            ...
            <div class="bg-white border border-amber-100 shadow rounded-lg p-4 mb-12">
                ...
                <div>
                    <label for="category" class="text-sm font-semibold text-amber-700 flex items-center gap-2 mb-1">
                        <i class="fas fa-search text-amber-500"></i>Buscar por Categoría
                    </label>
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('blog.index') }}"
                            class="text-sm px-4 py-1.5 rounded font-semibold transition-all duration-200
                      {{ request()->routeIs('blog.index') ? 'bg-amber-600 text-white shadow' : 'bg-amber-100 text-amber-800 hover:bg-amber-200' }}">
                            Todas las categorías
                        </a>
                        @foreach ($categories as $category)
                            <a href="{{ route('blog.category', ['category' => $category->slug]) }}"
                                class="text-sm px-4 py-1.5 rounded font-semibold transition-all duration-200
                      {{ request('category') === $category->slug ? 'bg-amber-600 text-white shadow' : 'bg-amber-100 text-amber-800 hover:bg-amber-200' }}">
                                {{ $category->name }}
                            </a>
                        @endforeach
                    </div>
                ...
                <div>
                    <label for="tag" class="text-sm font-semibold text-amber-700 flex items-center gap-2 mb-1">
                        <i class="fas fa-search text-amber-500"></i>Buscar por Etiquetas
                    </label>
                    <div class="flex flex-wrap gap-2">
                        @foreach ($tags as $tag)
                            <a href="{{ route('blog.tag', ['tag' => $tag->slug]) }}"
                                class="text-sm px-4 py-1.5 rounded font-medium transition-all duration-200
                      {{ request('tag') === $tag->slug ? 'bg-amber-600 text-white shadow' : 'bg-amber-100 text-amber-800 hover:bg-amber-200' }}">
                                <i class="fas fa-tags mr-2 text-amber-500"></i>{{ $tag->name }}
                            </a>
                        @endforeach
                    </div>
                </div>
