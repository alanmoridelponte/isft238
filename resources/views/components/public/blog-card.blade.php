    ...
    <div class="pt-3 px-4">
        <div class="flex items-center gap-2 mb-2 justify-between">
            <a href="{{ route('blog.category', ['category' => $post->category->slug]) }}"
                class="text-xs bg-amber-100 text-amber-800 font-semibold px-2 py-1 rounded">
                {{ $post->category->name }}
            </a>
        </div>
        <a href="{{ route('blog.show', ['category' => $post->category->slug, 'post' => $post->slug]) }}">
            <h2 class="text-xl font-bold text-amber-700 mb-2">{{ $post->title }}</h2>
            <p class="text-gray-700 text-sm mb-2">{{ $post->excerpt }}</p>
            <span class="text-amber-700 font-semibold no-underline">
